...
for(i in 1:length(site.val)){
  ...
  if(i==4){text(x=1998.5,y=ylim.val[1],"Total Phosphorus (\u03BCg L\u207B\u00B9)",xpd=NA,srt=90,cex=1.75,adj=0)}
}
...
for(i in 1:length(site.val)){
  ...
  axis_fun(2,ymaj,ymin,ymaj);box(lwd=1)
  mtext(side=3,site.val[i],cex=0.8)
  if(i==4){text(x=1998.5,y=ylim.val[1],"Total Nitrogen (mg L\u207B\u00B9)",xpd=NA,srt=90,cex=1.75,adj=0)}
}
for(j in 1:7){plot(0:1,0:1,axes=F,type="n",ylab=NA,xlab=NA)}
...
for(i in 1:length(site.val)){
  ...
  with(subset(WQ.WY,Station.ID==site.val[i]),points(WY,DOC.scn,pch=23,bg="darkgoldenrod1",cex=1.5))
  if(i==6|i==11){axis_fun(1,line=-0.5,xmaj,xmin,xmaj)}else(axis_fun(1,xmaj,xmin,NA))
  axis_fun(2,ymaj,ymin,ymaj);box(lwd=1)
  mtext(side=3,site.val[i],cex=0.8)
  if(i==5){text(x=1998.5,y=ylim.val[2],"Dissolved Organic Carbon (mg L\u207B\u00B9)",xpd=NA,srt=90,cex=1.75,adj=0)}
}
...
legend(0.5,0.75,legend=legend.text,pch =c(21,22,23,19),col=c("black","black","black",pt.cols[4]),pt.bg=pt.cols,pt.cex=1.5,ncol=1,cex=1,bty="n",y.intersp=1,x.intersp=0.75,xpd=NA,xjust=0.5)
dev.off()
...
for(i in 1:length(site.val)){
  ...
  with(subset(WQ.WY,Station.ID==site.val[i]),points(WY,SRP.scn,pch=21,bg="sienna1",cex=1.5))
  if(i==6|i==11){axis_fun(1,line=-0.5,xmaj,xmin,xmaj)}else(axis_fun(1,xmaj,xmin,NA))
  axis_fun(2,ymaj,ymin,ymaj);box(lwd=1)
  mtext(side=3,site.val[i],cex=0.8)
  if(i==5){text(x=1999.5,y=ylim.val[2],"Soluable Reactive Phosphorus (\u03BCg L\u207B\u00B9)",xpd=NA,srt=90,cex=1.75,adj=0)}
}
for(j in 1:7){plot(0:1,0:1,axes=F,type="n",ylab=NA,xlab=NA)}
...
for(i in 1:length(site.val)){
  ...
  with(subset(WQ.WY,Station.ID==site.val[i]),points(WY,DIN.scn,pch=22,bg="darkcyan",cex=1.5))
  if(i==6|i==11){axis_fun(1,line=-0.5,xmaj,xmin,xmaj)}else(axis_fun(1,xmaj,xmin,NA))
  axis_fun(2,ymaj,ymin,ymaj);box(lwd=1)
  mtext(side=3,site.val[i],cex=0.8)
  if(i==5){text(x=1999,y=ylim.val[2],"Dissolved Inorganic Nitrogen (mg L\u207B\u00B9)",xpd=NA,srt=90,cex=1.75,adj=0)}
}
...
#CSF models with .y = year as a random effect
mnull.WY.TP.SRS<-lme(log(TP.scn)~1,data=subset(WQ.WY2.TP,Region=="SRS"),random=list(~1|Station.ID,~1|WY.f),method="ML")
m1.WY.TP.SRS=lme(log(TP.scn)~ACE,data=subset(WQ.WY2.TP,Region=="SRS"),random=list(~1|Station.ID,~1|WY.f),method="ML")
m2.WY.TP.SRS<-update(m1.WY.TP.SRS, .~. +I(ACE^2))
m3.WY.TP.SRS<-update(m2.WY.TP.SRS, .~. +I(ACE^3))
m1AR.WY.TP.SRS<-lme(log(TP.scn)~ACE,data=subset(WQ.WY2.TP,Region=="SRS"),random=list(~1|Station.ID,~1|WY.f),correlation=corAR1(form=~WY),method="ML")
m2AR.WY.TP.SRS<-update(m1AR.WY.TP.SRS, .~. +I(ACE^2))
m3AR.WY.TP.SRS<-update(m2AR.WY.TP.SRS, .~. +I(ACE^3))

#Table SX: Model selection without year
AICc.TP.SRS=AICc(mnull.TP.SRS,m1.TP.SRS, m2.TP.SRS, m3.TP.SRS,m1AR.TP.SRS,m2AR.TP.SRS,m3AR.TP.SRS)
AICc.TP.SRS
subset(AICc.TP.SRS,AICc==min(AICc.TP.SRS$AICc))

# Table SX: Model selection set year as random
AICc.WY.TP.SRS=AICc(mnull.WY.TP.SRS,m1.WY.TP.SRS, m2.WY.TP.SRS, m3.WY.TP.SRS,m1AR.WY.TP.SRS,m2AR.WY.TP.SRS,m3AR.WY.TP.SRS)
AICc.WY.TP.SRS
subset(AICc.WY.TP.SRS,AICc==min(AICc.WY.TP.SRS$AICc))

#m2.TP.SRS model is the best.

#Table SX: Best model, p-values and coefficient estimates random year effect
Anova(m2.TP.SRS,type=3)
summary(m2.TP.SRS)
rsquared(m2.TP.SRS)
...
qqnorm(residuals(m2.TP.SRS));qqline(residuals(m2.TP.SRS))
shapiro.test(residuals(m2.TP.SRS));
...
#CSF models with .y = year as a random effect
mnull.WY.TP.TS<-lme(log(TP.scn)~1,data=subset(WQ.WY2.TP,Region=="TS"),random=list(~1|Station.ID,~1|WY.f),method="ML")
m1.WY.TP.TS=lme(log(TP.scn)~ACE,data=subset(WQ.WY2.TP,Region=="TS"),random=list(~1|Station.ID,~1|WY.f),method="ML")
m2.WY.TP.TS<-update(m1.WY.TP.TS, .~. +I(ACE^2))
m3.WY.TP.TS<-update(m2.WY.TP.TS, .~. +I(ACE^3))
m1AR.WY.TP.TS<-lme(log(TP.scn)~ACE,data=subset(WQ.WY2.TP,Region=="TS"),random=list(~1|Station.ID,~1|WY.f),correlation=corAR1(form=~WY),method="ML")
m2AR.WY.TP.TS<-update(m1AR.WY.TP.TS, .~. +I(ACE^2))
m3AR.WY.TP.TS<-update(m2AR.WY.TP.TS, .~. +I(ACE^3))

#Table SX: Model selection without year
AICc.TP.TS=AICc(mnull.TP.TS,m1.TP.TS, m2.TP.TS, m3.TP.TS,m1AR.TP.TS,m2AR.TP.TS,m3AR.TP.TS)
#AICc.TP.TS
subset(AICc.TP.TS,AICc==min(AICc.TP.TS$AICc))

# Table SX: Model selection set year as random
AICc.WY.TP.TS=AICc(mnull.WY.TP.TS,m1.WY.TP.TS, m2.WY.TP.TS, m3.WY.TP.TS,m1AR.WY.TP.TS,m2AR.WY.TP.TS,m3AR.WY.TP.TS)
#AICc.WY.TP.TS
subset(AICc.WY.TP.TS,AICc==min(AICc.WY.TP.TS$AICc))

#m2.TP.TS model is the best.
...
#2) Normality of Residuals (of best model)
qqnorm(residuals(m2.TP.TS));qqline(residuals(m2.TP.TS))

shapiro.test(residuals(m2.TP.TS));

#Quick Plot
m2.TP.TS.model=visreg(m2.TP.TS,"ACE",trans=exp,type="conditional",plot=F)
m2.TP.SRS.model=visreg(m2.TP.SRS,"ACE",trans=exp,type="conditional",plot=F)

plot(TP.scn~ACE,WQ.WY2.TP,pch=ifelse(Region=="SRS",21,22),bg=ifelse(Region=="SRS","indianred1","dodgerblue1"))
with(m2.TP.TS.model$fit,shaded.range(ACE,visregLwr,visregUpr,"dodgerblue1",lty=1))
with(m2.TP.TS.model$fit,lines(ACE,visregFit,col="dodgerblue1",lwd=2))
with(m2.TP.SRS.model$fit,shaded.range(ACE,visregLwr,visregUpr,"indianred1",lty=1))
with(m2.TP.SRS.model$fit,lines(ACE,visregFit,col="indianred1",lwd=2))

# Total Nitogen Model
# Shark River Slough
mnull.TN.SRS<-lme(log(TN.scn)~1,data=subset(WQ.WY2.TN,Region=="SRS"),random=~1|Station.ID,method="ML")
m1.TN.SRS=lme(log(TN.scn)~ACE,data=subset(WQ.WY2.TN,Region=="SRS"),random=~1|Station.ID,method="ML")
m2.TN.SRS<-update(m1.TN.SRS, .~. +I(ACE^2))
m3.TN.SRS<-update(m2.TN.SRS, .~. +I(ACE^3))
m1AR.TN.SRS<-lme(log(TN.scn)~ACE,data=subset(WQ.WY2.TN,Region=="SRS"),random=~1|Station.ID,correlation=corAR1(form=~WY),method="ML")
m2AR.TN.SRS<-update(m1AR.TN.SRS, .~. +I(ACE^2))
m3AR.TN.SRS<-update(m2AR.TN.SRS, .~. +I(ACE^3))

#CSF models with .y = year as a random effect
mnull.WY.TN.SRS<-lme(log(TN.scn)~1,data=subset(WQ.WY2.TN,Region=="SRS"),random=list(~1|Station.ID,~1|WY.f),method="ML")
m1.WY.TN.SRS=lme(log(TN.scn)~ACE,data=subset(WQ.WY2.TN,Region=="SRS"),random=list(~1|Station.ID,~1|WY.f),method="ML")
m2.WY.TN.SRS<-update(m1.WY.TN.SRS, .~. +I(ACE^2))
m3.WY.TN.SRS<-update(m2.WY.TN.SRS, .~. +I(ACE^3))
m1AR.WY.TN.SRS<-lme(log(TN.scn)~ACE,data=subset(WQ.WY2.TN,Region=="SRS"),random=list(~1|Station.ID,~1|WY.f),correlation=corAR1(form=~WY),method="ML")
m2AR.WY.TN.SRS<-update(m1AR.WY.TN.SRS, .~. +I(ACE^2))
m3AR.WY.TN.SRS<-update(m2AR.WY.TN.SRS, .~. +I(ACE^3))

#Table SX: Model selection without year
AICc.TN.SRS=AICc(mnull.TN.SRS,m1.TN.SRS, m2.TN.SRS, m3.TN.SRS,m1AR.TN.SRS,m2AR.TN.SRS,m3AR.TN.SRS)
#AICc.TN.SRS
subset(AICc.TN.SRS,AICc==min(AICc.TN.SRS$AICc))

# Table SX: Model selection set year as random
AICc.WY.TN.SRS=AICc(mnull.WY.TN.SRS,m1.WY.TN.SRS, m2.WY.TN.SRS, m3.WY.TN.SRS,m1AR.WY.TN.SRS,m2AR.WY.TN.SRS,m3AR.WY.TN.SRS)
#AICc.WY.TN.SRS
subset(AICc.WY.TN.SRS,AICc==min(AICc.WY.TN.SRS$AICc))

#m2AR.TN.SRS model is the best.

#Table SX: Best model, p-values and coefficient estimates random year effect
Anova(m2AR.TN.SRS,type=3)
summary(m2AR.TN.SRS)
rsquared(m2AR.TN.SRS)
...
#2) Normality of Residuals (of best model)
qqnorm(residuals(m2AR.TN.SRS));qqline(residuals(m2AR.TN.SRS))
shapiro.test(residuals(m2AR.TN.SRS));

# Taylor Slough
mnull.TN.TS<-lme(log(TN.scn)~1,data=subset(WQ.WY2.TN,Region=="TS"),random=~1|Station.ID,method="ML")
m1.TN.TS=lme(log(TN.scn)~ACE,data=subset(WQ.WY2.TN,Region=="TS"),random=~1|Station.ID,method="ML")
m2.TN.TS<-update(m1.TN.TS, .~. +I(ACE^2))
m3.TN.TS<-update(m2.TN.TS, .~. +I(ACE^3))
m1AR.TN.TS<-lme(log(TN.scn)~ACE,data=subset(WQ.WY2.TN,Region=="TS"),random=~1|Station.ID,correlation=corAR1(form=~WY),method="ML")
m2AR.TN.TS<-update(m1AR.TN.TS, .~. +I(ACE^2))
m3AR.TN.TS<-update(m2AR.TN.TS, .~. +I(ACE^3))

#CSF models with .y = year as a random effect
mnull.WY.TN.TS<-lme(log(TN.scn)~1,data=subset(WQ.WY2.TN,Region=="TS"),random=list(~1|Station.ID,~1|WY.f),method="ML")
m1.WY.TN.TS=lme(log(TN.scn)~ACE,data=subset(WQ.WY2.TN,Region=="TS"),random=list(~1|Station.ID,~1|WY.f),method="ML")
m2.WY.TN.TS<-update(m1.WY.TN.TS, .~. +I(ACE^2))
m3.WY.TN.TS<-update(m2.WY.TN.TS, .~. +I(ACE^3))
m1AR.WY.TN.TS<-lme(log(TN.scn)~ACE,data=subset(WQ.WY2.TN,Region=="TS"),random=list(~1|Station.ID,~1|WY.f),correlation=corAR1(form=~WY),method="ML")
m2AR.WY.TN.TS<-update(m1AR.WY.TN.TS, .~. +I(ACE^2))
m3AR.WY.TN.TS<-update(m2AR.WY.TN.TS, .~. +I(ACE^3))

#Table SX: Model selection without year
AICc.TN.TS=AICc(mnull.TN.TS,m1.TN.TS, m2.TN.TS, m3.TN.TS,m1AR.TN.TS,m2AR.TN.TS,m3AR.TN.TS)
#AICc.TN.TS
subset(AICc.TN.TS,AICc==min(AICc.TN.TS$AICc))

# Table SX: Model selection set year as random
AICc.WY.TN.TS=AICc(mnull.WY.TN.TS,m1.WY.TN.TS, m2.WY.TN.TS, m3.WY.TN.TS,m1AR.WY.TN.TS,m2AR.WY.TN.TS,m3AR.WY.TN.TS)
#AICc.WY.TN.TS
subset(AICc.WY.TN.TS,AICc==min(AICc.WY.TN.TS$AICc))

#m1AR.TN.TS model is the best.

#Table SX: Best model, p-values and coefficient estimates random year effect
Anova(m1AR.TN.TS,type=3)
summary(m1AR.TN.TS)
rsquared(m1AR.TN.TS)
...
#2) Normality of Residuals (of best model)
qqnorm(residuals(m1AR.TN.TS));qqline(residuals(m1AR.TN.TS))

shapiro.test(residuals(m1AR.TN.TS));

#Quick plot
m1AR.TN.TS.model=visreg(m1AR.TN.TS,"ACE",trans=exp,type="conditional",plot=F)
m2AR.TN.SRS.model=visreg(m2AR.TN.SRS,"ACE",trans=exp,type="conditional",plot=F)

plot(TN.scn~ACE,WQ.WY2.TP,pch=ifelse(Region=="SRS",21,22),bg=ifelse(Region=="SRS","indianred1","dodgerblue1"))
with(m1AR.TN.TS.model$fit,shaded.range(ACE,visregLwr,visregUpr,"dodgerblue1",lty=1))
with(m1AR.TN.TS.model$fit,lines(ACE,visregFit,col="dodgerblue1",lwd=2))
with(m2AR.TN.SRS.model$fit,shaded.range(ACE,visregLwr,visregUpr,"indianred1",lty=1))
with(m2AR.TN.SRS.model$fit,lines(ACE,visregFit,col="indianred1",lwd=2))


# Dissolved Organic Carbon
# Shark River Slough
mod.dat=subset(WQ.WY2.DOC,Region=="SRS")

mnull.DOC.SRS<-lme(log(DOC.scn)~1,data=mod.dat,random=~1|Station.ID,method="ML")
m1.DOC.SRS=lme(log(DOC.scn)~ACE,data=mod.dat,random=~1|Station.ID,method="ML")
m2.DOC.SRS<-update(m1.DOC.SRS, .~. +I(ACE^2))
m3.DOC.SRS<-update(m2.DOC.SRS, .~. +I(ACE^3))
m1AR.DOC.SRS<-lme(log(DOC.scn)~ACE,data=mod.dat,random=~1|Station.ID,correlation=corAR1(form=~WY),method="ML")
m2AR.DOC.SRS<-update(m1AR.DOC.SRS, .~. +I(ACE^2))
m3AR.DOC.SRS<-update(m2AR.DOC.SRS, .~. +I(ACE^3))

#CSF models with .y = year as a random effect
mnull.WY.DOC.SRS<-lme(log(DOC.scn)~1,data=mod.dat,random=list(~1|Station.ID,~1|WY.f),method="ML")
m1.WY.DOC.SRS=lme(log(DOC.scn)~ACE,data=mod.dat,random=list(~1|Station.ID,~1|WY.f),method="ML")
m2.WY.DOC.SRS<-update(m1.WY.DOC.SRS, .~. +I(ACE^2))
m3.WY.DOC.SRS<-update(m2.WY.DOC.SRS, .~. +I(ACE^3))
m1AR.WY.DOC.SRS<-lme(log(DOC.scn)~ACE,data=mod.dat,random=list(~1|Station.ID,~1|WY.f),correlation=corAR1(form=~WY),method="ML")
m2AR.WY.DOC.SRS<-update(m1AR.WY.DOC.SRS, .~. +I(ACE^2))
m3AR.WY.DOC.SRS<-update(m2AR.WY.DOC.SRS, .~. +I(ACE^3))

#Table SX: Model selection without year
AICc.DOC.SRS=AICc(mnull.DOC.SRS,m1.DOC.SRS, m2.DOC.SRS, m3.DOC.SRS,m1AR.DOC.SRS,m2AR.DOC.SRS,m3AR.DOC.SRS)
#AICc.DOC.SRS
subset(AICc.DOC.SRS,AICc==min(AICc.DOC.SRS$AICc))

# Table SX: Model selection set year as random
AICc.WY.DOC.SRS=AICc(mnull.WY.DOC.SRS,m1.WY.DOC.SRS, m2.WY.DOC.SRS, m3.WY.DOC.SRS,m1AR.WY.DOC.SRS,m2AR.WY.DOC.SRS,m3AR.WY.DOC.SRS)
#AICc.WY.DOC.SRS
subset(AICc.WY.DOC.SRS,AICc==min(AICc.WY.DOC.SRS$AICc))

#m1.WY.DOC.SRS model is the best.

#Table SX: Best model, p-values and coefficient estimates random year effect
Anova(m1.WY.DOC.SRS,type=3)
summary(m1.WY.DOC.SRS)
rsquared(m1.WY.DOC.SRS)

plot(m1.WY.DOC.SRS)
qqnorm(residuals(m1.WY.DOC.SRS));qqline(residuals(m1.WY.DOC.SRS))
shapiro.test(residuals(m1.WY.DOC.SRS));

# Taylor Slough
mod.dat=subset(WQ.WY2.DOC,Region=="TS")

mnull.DOC.TS<-lme(log(DOC.scn)~1,data=mod.dat,random=~1|Station.ID,method="ML")
m1.DOC.TS=lme(log(DOC.scn)~ACE,data=mod.dat,random=~1|Station.ID,method="ML")
m2.DOC.TS<-update(m1.DOC.TS, .~. +I(ACE^2))
m3.DOC.TS<-update(m2.DOC.TS, .~. +I(ACE^3))
m1AR.DOC.TS<-lme(log(DOC.scn)~ACE,data=mod.dat,random=~1|Station.ID,correlation=corAR1(form=~WY),method="ML")
m2AR.DOC.TS<-update(m1AR.DOC.TS, .~. +I(ACE^2))
m3AR.DOC.TS<-update(m2AR.DOC.TS, .~. +I(ACE^3))

#CSF models with .y = year as a random effect
mnull.WY.DOC.TS<-lme(log(DOC.scn)~1,data=mod.dat,random=list(~1|Station.ID,~1|WY.f),method="ML")
m1.WY.DOC.TS=lme(log(DOC.scn)~ACE,data=mod.dat,random=list(~1|Station.ID,~1|WY.f),method="ML")
m2.WY.DOC.TS<-update(m1.WY.DOC.TS, .~. +I(ACE^2))
m3.WY.DOC.TS<-update(m2.WY.DOC.TS, .~. +I(ACE^3))
m1AR.WY.DOC.TS<-lme(log(DOC.scn)~ACE,data=mod.dat,random=list(~1|Station.ID,~1|WY.f),correlation=corAR1(form=~WY),method="ML")
m2AR.WY.DOC.TS<-update(m1AR.WY.DOC.TS, .~. +I(ACE^2))
m3AR.WY.DOC.TS<-update(m2AR.WY.DOC.TS, .~. +I(ACE^3))

#Table SX: Model selection without year
AICc.DOC.TS=AICc(mnull.DOC.TS,m1.DOC.TS, m2.DOC.TS, m3.DOC.TS,m1AR.DOC.TS,m2AR.DOC.TS,m3AR.DOC.TS)
#AICc.DOC.TS
subset(AICc.DOC.TS,AICc==min(AICc.DOC.TS$AICc))

# Table SX: Model selection set year as random
AICc.WY.DOC.TS=AICc(mnull.WY.DOC.TS,m1.WY.DOC.TS, m2.WY.DOC.TS, m3.WY.DOC.TS,m1AR.WY.DOC.TS,m2AR.WY.DOC.TS,m3AR.WY.DOC.TS)
#AICc.WY.DOC.TS
subset(AICc.WY.DOC.TS,AICc==min(AICc.WY.DOC.TS$AICc))

#m1AR.DOC.TS model is the best.

#Table SX: Best model, p-values and coefficient estimates random year effect
Anova(m1.WY.DOC.TS,type=3)
summary(m1.WY.DOC.TS)
rsquared(m1.WY.DOC.TS)


plot(m1.WY.DOC.TS)
qqnorm(residuals(m1.WY.DOC.TS));qqline(residuals(m1.WY.DOC.TS))
shapiro.test(residuals(m1.WY.DOC.TS));

#Quick plot
m1.WY.DOC.TS.model=visreg(m1AR.WY.DOC.TS,"ACE",trans=exp,type="conditional",plot=F)
m1.WY.DOC.SRS.model=visreg(m1.WY.DOC.SRS,"ACE",trans=exp,type="conditional",plot=F)

plot(DOC.scn~ACE,WQ.WY2.TP,pch=ifelse(Region=="SRS",21,22),bg=ifelse(Region=="SRS","indianred1","dodgerblue1"))
with(m1AR.DOC.TS.model$fit,shaded.range(ACE,visregLwr,visregUpr,"dodgerblue1",lty=1))
with(m1AR.DOC.TS.model$fit,lines(ACE,visregFit,col="dodgerblue1",lwd=2))
with(m1AR.DOC.SRS.model$fit,shaded.range(ACE,visregLwr,visregUpr,"indianred1",lty=1))
with(m1AR.DOC.SRS.model$fit,lines(ACE,visregFit,col="indianred1",lwd=2))
